...
function drawLockOverlayScene() {
  ...
  const paint = () => {
    ...
    for (let i = 0; i < 92; i += 1) {
      ...
      ctx.fillStyle = `rgba(222, 243, 255, ${Math.min(0.95, twinkle).toFixed(3)})`;
      ctx.beginPath();
    ...
    for (let i = 0; i < 5; i += 1) {
      ...
      ctx.strokeStyle = `rgba(120, 214, 255, ${alpha.toFixed(3)})`;
    ...
    const fishSchool = [
      { speed: 0.14, depth: 0.08, size: 1.0, dir: 1, colorA: 'rgba(114, 236, 255, 0.95)', colorB: 'rgba(116, 160, 255, 0.86)' },
      { speed: 0.11, depth: 0.18, size: 0.82, dir: -1, colorA: 'rgba(255, 188, 132, 0.95)', colorB: 'rgba(247, 107, 165, 0.82)' },
      { speed: 0.17, depth: 0.25, size: 0.72, dir: 1, colorA: 'rgba(146, 255, 214, 0.95)', colorB: 'rgba(77, 206, 255, 0.84)' },
      { speed: 0.095, depth: 0.33, size: 1.18, dir: -1, colorA: 'rgba(174, 205, 255, 0.94)', colorB: 'rgba(122, 139, 255, 0.86)' }
    ];
    ...
    for (let i = 0; i < fishSchool.length; i += 1) {
      ...
      for (let b = 0; b < 5; b += 1) {
        ...
        if (alpha <= 0) continue;
        ctx.strokeStyle = `rgba(224, 247, 255, ${alpha.toFixed(3)})`;
    ...
    ctx.shadowColor = `rgba(119, 222, 255, ${glow.toFixed(3)})`;
    ...
    for (let i = 0; i < passCount; i += 1) {
      ...
      ctx.rotate(tilt);
      ctx.strokeStyle = `rgba(6, 22, 48, ${(0.48 + i * 0.06).toFixed(3)})`;
      ctx.fillStyle = `rgba(255, 249, 222, ${Math.min(0.98, alpha).toFixed(3)})`;
